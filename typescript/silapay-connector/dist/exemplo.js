...
async function main() {
    const pay = new index_1.PaymentConnector({ provider: "silapay" });
    console.log("Saldo:");
    console.log(await pay.saldo());
    console.log("\nPIX CashIn:");
    console.log(await pay.pix({
        cashFlowType: "cashIn",
        value: 50,
    ...
    const pay2 = new index_1.PaymentConnector({ provider: "mercadopago" });
...
main().catch((error) => {
    const responseData = typeof error === "object" && error !== null && "responseData" in error
        ? error.responseData
        : undefined;
